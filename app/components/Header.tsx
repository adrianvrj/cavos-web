		...
		<>
			<header className="container mx-auto px-6 py-6 flex justify-between items-center backdrop-blur-sm relative z-50">
				<div className="flex items-center gap-2">
					...
					<h1 className="font-bold text-3xl text-[#EAE5DC] tracking-tight">
						CAVOS
					</h1>
				</div>
				...
				<nav className="hidden md:flex items-center gap-8">
					<a
						href="/"
						className="hover:text-[#EAE5DC] transition-all duration-300 relative group"
					>
						Home
				...
				>
					<span
						className={`w-6 h-0.5 bg-[#EAE5DC] transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`}
					></span>
					<span
						className={`w-6 h-0.5 bg-[#EAE5DC] transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''}`}
					></span>
					<span
						className={`w-6 h-0.5 bg-[#EAE5DC] transition-all duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}
					></span>
			...
			{/* Mobile Menu Panel */}
			<div
				className={`fixed top-0 right-0 h-full w-80 max-w-[80vw] bg-gradient-to-b from-black/95 to-black/90 backdrop-blur-xl z-50 md:hidden transition-transform duration-300 ease-in-out ${
